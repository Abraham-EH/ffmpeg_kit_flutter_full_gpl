...
class FFmpegKitConfig {
  ...
  /// Note that you need to use a package with "fontconfig" inside to be able
  /// to use fonts in "FFmpeg".
  static Future<void> setFontDirectoryList(List<String> fontDirectoryList, [Map<String, String>? mapping = null]) async {
    try {
      await init();
  ...
  static Future<void> getMediaInformationExecute(MediaInformationSession mediaInformationSession, [int? waitTimeout = null]) async {
    try {
      await init();
  ...
  static Future<void> asyncGetMediaInformationExecute(MediaInformationSession mediaInformationSession, [int? waitTimeout = null]) async {
    try {
      await init();
      return _platform.ffmpegKitConfigAsyncGetMediaInformationExecute(mediaInformationSession.getSessionId(), waitTimeout);
    } on PlatformException catch (e, stack) {
  ...
  /// for FFmpeg sessions.
  static void enableFFmpegSessionCompleteCallback([FFmpegSessionCompleteCallback? ffmpegSessionCompleteCallback = null]) {
  ...
  static FFmpegSessionCompleteCallback? getFFmpegSessionCompleteCallback() => FFmpegKitFactory.getGlobalFFmpegSessionCompleteCallback();

  /// Sets a global FFprobeSessionCompleteCallback to receive execution results
  /// for FFprobe sessions.
  static void enableFFprobeSessionCompleteCallback([FFprobeSessionCompleteCallback? ffprobeSessionCompleteCallback = null]) {
  ...
  static FFprobeSessionCompleteCallback? getFFprobeSessionCompleteCallback() => FFmpegKitFactory.getGlobalFFprobeSessionCompleteCallback();

  /// Sets a global MediaInformationSessionCompleteCallback to receive
  /// execution results for MediaInformation sessions.
  static void enableMediaInformationSessionCompleteCallback([MediaInformationSessionCompleteCallback? mediaInformationSessionCompleteCallback = null]) {
  ...
  static MediaInformationSessionCompleteCallback? getMediaInformationSessionCompleteCallback() => FFmpegKitFactory.getGlobalMediaInformationSessionCompleteCallback();
    ...
    try {
      ...
      return _platform.ffmpegKitConfigGetSessions().then((sessions) {
        if (sessions == null) {
          ...
          return sessions.map((dynamic sessionObject) => FFmpegKitFactory.mapToSession(sessionObject as Map<dynamic, dynamic>)).toList();
        }
      });
    } on PlatformException catch (e, stack) {
    ...
    try {
      ...
      return _platform.ffmpegKitListSessions().then((sessions) {
        if (sessions == null) {
          ...
          return sessions.map((dynamic sessionObject) => FFmpegKitFactory.mapToSession(sessionObject as Map<dynamic, dynamic>)).map((session) => session as FFmpegSession).toList();
        }
      });
    } on PlatformException catch (e, stack) {
    ...
    try {
      ...
      return _platform.ffprobeKitListFFprobeSessions().then((sessions) {
        if (sessions == null) {
          ...
          return sessions.map((dynamic sessionObject) => FFmpegKitFactory.mapToSession(sessionObject as Map<dynamic, dynamic>)).map((session) => session as FFprobeSession).toList();
        }
      });
    } on PlatformException catch (e, stack) {
    ...
    try {
      ...
      return _platform.ffprobeKitListMediaInformationSessions().then((sessions) {
        if (sessions == null) {
          ...
          return sessions.map((dynamic sessionObject) => FFmpegKitFactory.mapToSession(sessionObject as Map<dynamic, dynamic>)).map((session) => session as MediaInformationSession).toList();
        }
      });
    } on PlatformException catch (e, stack) {
  ...
  /// Note that this method is Android only. It will fail if called on other
  /// platforms.
  static Future<String?> selectDocumentForWrite([String? title = null, String? type = null, List<String>? extraTypes = null]) async {
    try {
      await init();
